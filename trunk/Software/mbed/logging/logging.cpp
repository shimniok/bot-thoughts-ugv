...
void logData( const SystemState s )
{
	...
	sprintf(buf, "%d,%.2f,%.2f,%d,%d,%d,%d,%d,%d,%d,%.2f,%.7f,%.7f,%.2f,%.2f,%.2f,%d,%.7f,%.7f,%.2f,%.2f,%.2f,%.2f,%.7f,%.7f,%.4f,%.4f,%d,%.2f,%.3f,",
			s.millis,
			s.current,
			s.voltage,
			s.g[0], s.g[1], s.g[2],
			...
			s.gpsCourse_deg, s.gpsSpeed_mps,
			s.gpsHDOP, s.gpsSats,
			s.lrEncDistance, s.rrEncDistance,
			s.lrEncSpeed, s.rrEncSpeed,
			s.encHeading, s.estHeading,
			s.estLatitude, s.estLongitude,
			s.estX, s.estY,
...
void logData2( const SystemState s )
{
	//char buf[256];
	unsigned int t1, t2;
	logtimer.start();
